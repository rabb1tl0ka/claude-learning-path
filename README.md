...
| Skill | What it does |
|---|---|
| `/learning-recap` | Turns a Gemini transcript of a self-recorded "watch the course + narrate" session into a chapter note (Summary / My Insights / Ideas / Challenges / Actions). Saves to `2ndbrain/ccaf-learning/<course>/<section>/<chapter>.md` (section folder only if the course's `.course-toc.json` tags one; otherwise `2ndbrain/ccaf-learning/<course>/<chapter>.md`). Any `owner: claude` research note triggered mid-session goes one level deeper, in that same location's `research/` subdirectory. Also updates `2ndbrain/CLAUDE.md`'s Progress section. |
| `/flashcards` | Quizzes you with multiple-choice flashcards generated from the chapter and gap-topic notes; tracks weak spots over time. Also imports missed questions from a real mock-exam file to seed weak spots and generate a targeted study guide (`/flashcards import <path>`). Runs from `2ndbrain/` as cwd; caches question banks under `2ndbrain/.flashcards/cards/<course>/`, tracks weak spots in `2ndbrain/.flashcards/weak-spots.md` and `history.jsonl`, and writes imported study guides to `2ndbrain/.flashcards/study-guides/`. Per-session results (this repo's config) go to `2ndbrain/exam-prep/flashcards/flashcards-results-<date>.md`. |
| `/exam-analysis` | Reads a mock-exam result file (dropped in `2ndbrain/exam-prep/attempts/`) and reports *why* you're missing questions (recurring reasoning mistakes, not just which domain scored low) plus what you consistently get right. Flags real coverage gaps — questions that don't map to any existing note. Saves the analysis to `2ndbrain/exam-prep/attempts/<source-slug>-<date>-analysis.md`; cross-cutting reasoning heuristics (patterns that span topics, not single-topic gaps) get drafted or updated in `2ndbrain/exam-prep/study-guide-weak-spots/notes/exam-prep-survival-guide.md`. Doesn't touch `.flashcards/` itself — recommends running `/flashcards import` on the same file as a follow-up. |
